...
def objective(trial):
    ...
    hparams = {
        ...
        'hnet_d_model': trial.suggest_categorical('hnet_d_model', 
            ['96_192', '128_256', '192_384']),  # Will parse below
        ...
        'hnet_ssm_d_state': trial.suggest_categorical('hnet_ssm_d_state', [16,32, 64, 128]),
        ...
        'learning_rate': trial.suggest_categorical('learning_rate',  [0.0001, 0.0025, 0.0005, 0.001]),
    ...
    for horizon_idx, pred_len in enumerate(HORIZON_LENGTHS):
        ...
        cmd_args.extend(['--model_id', f'ETTm1_96_{pred_len}'])
...
def save_study_results(study, output_file='hpo_results_ettm1_optuna.json'):
...
if __name__ == '__main__':
    ...
    print(f"{'='*80}")
    print(f"Starting Optuna HPO for ByteHnet on ETTm1")
    print(f"{'='*80}")
    ...
    output_path = os.path.join(hpo_dir, 'hpo_results_ettm1_optuna.json')
