...
# A pre-norm residual block that optionally includes an MLP after the mixer.
class Block(nn.Module):
    ...
    def forward(
        ...
        # Pre-norm + mixer + optional (pre-norm + MLP)
        ...
        hidden_states = self.mixer(
            hidden_states.contiguous(), inference_params=inference_params, **mixer_kwargs
        )
    ...
    def step(self, hidden_states, inference_params, residual=None):
        ...
        hidden_states = self.mixer.step(hidden_states.contiguous(), inference_params)
        if self.mlp is not None:
            hidden_states, residual = self.norm2(
                hidden_states,
                residual=residual,
